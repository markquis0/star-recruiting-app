...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Track public profile view with Mixpanel
        if (typeof trackEvent === 'function') {
            const profileToken = '{{ request()->route("token") ?? "" }}';
            trackEvent('Public Profile Viewed', {
                candidate_id: {{ $candidate->id }},
                candidate_name: '{{ addslashes($candidate->first_name . ' ' . $candidate->last_name) }}',
                candidate_role_title: '{{ addslashes($candidate->role_title ?? '') }}',
                has_aptitude_profile: {{ $aptitudeProfile ? 'true' : 'false' }},
                has_behavioral_assessment: {{ $behavioralAssessment ? 'true' : 'false' }},
                projects_count: {{ $projects->count() }},
                profile_token: profileToken,
                is_short_code: profileToken.length <= 8,
            });
        }
    });
</script>
